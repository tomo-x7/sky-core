...
import { STARTERPACK_HEIGHT, STARTERPACK_WIDTH, StarterPack } from "../components/StarterPack.js";
	...
	return app.get(
		...
		handler(async (req, res) => {
			...
			try {
				...
				httpLogger.warn({ err, uri: uri.toString() }, "could not fetch starter pack");
				...
				[starterPack.creator]
					...
					.map(async (p) => {
						try {
							...
							httpLogger.warn({ err, uri: uri.toString(), did: p.did }, "could not fetch image");
							return [p.did, null] as const;
						}
					}),
			);
			const images = new Map(imageEntries.filter(([image]) => image !== null).slice(0, 7));
			const svg = await satori(<StarterPack starterPack={starterPack} images={images} />, {
				fonts: ctx.fonts,
