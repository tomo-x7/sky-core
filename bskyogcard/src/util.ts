...
export async function loadEmojiAsSvg(chars: string) {
	...
	const iconCode = twemoji.convert.toCodePoint(chars.indexOf(U200D) < 0 ? chars.replace(UFE0F_REGEXP, "") : chars);
	const res = await fetch(getEmojiUrl(iconCode));
	const body = await res.arrayBuffer();
	if (!res.ok) {
		renderLogger.warn({ status: res.status, err: Buffer.from(body).toString() }, "could not fetch emoji");
		return;
	}
	const svg = "data:image/svg+xml;base64," + Buffer.from(body).toString("base64");
