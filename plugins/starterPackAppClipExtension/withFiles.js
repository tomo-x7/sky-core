...
const withFiles = (config, { targetName }) => {
	...
	return withXcodeProject(config, (config) => {
		const basePath = path.join(config.modRequest.projectRoot, "modules", targetName);

		for (const file of FILES) {
			const sourcePath = path.join(basePath, file);
			const targetPath = path.join(config.modRequest.platformProjectRoot, targetName, file);
		...
		const imagesTargetPath = path.join(config.modRequest.platformProjectRoot, targetName, "Images.xcassets");
