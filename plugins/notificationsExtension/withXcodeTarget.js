...
const withXcodeTarget = (config, { extensionName, controllerName, soundFiles }) => {
	...
	return withXcodeProject(config, (config) => {
		...
		pbxProject.addSourceFile(`${extensionName}/${controllerName}.swift`, { target: target.uuid }, pbxGroupKey);

		for (const file of soundFiles) {
			pbxProject.addSourceFile(`${extensionName}/${file}`, { target: target.uuid }, pbxGroupKey);
		}
		...
		pbxProject.addTargetAttribute("DevelopmentTeam", "B3LX46C5HS", extensionName);
