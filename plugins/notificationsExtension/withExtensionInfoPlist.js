...
const withExtensionInfoPlist = (config, { extensionName }) => {
	...
	return withInfoPlist(config, (config) => {
		const plistPath = path.join(config.modRequest.projectRoot, "modules", extensionName, "Info.plist");
		const targetPath = path.join(config.modRequest.platformProjectRoot, extensionName, "Info.plist");
