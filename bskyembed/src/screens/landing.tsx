...
import { AppBskyFeedDefs, AppBskyFeedPost, AtUri, AtpAgent } from "@atproto/api";
import { render } from "preact";
import { useEffect, useMemo, useRef, useState } from "preact/hooks";

import arrowBottom from "../../assets/arrowBottom_stroke2_corner0_rounded.svg";
import logo from "../../assets/logo.svg";
import { type ColorModeValues, assertColorModeValues, initSystemColorMode } from "../color-mode";
...
const DEFAULT_URI = "at://did:plc:vjug55kidv6sye7ykr5faxxn/app.bsky.feed.post/3jzn6g7ixgq2y";
...
function LandingPage() {
	...
	return (
		<main className="w-full min-h-screen flex flex-col items-center gap-8 py-14 px-4 md:pt-32 dark:bg-dimmedBgDarken dark:text-slate-200">
			<Link href="https://bsky.social/about" className="transition-transform hover:scale-110">
			...
			{loading ? (
				...
				<div className="w-full max-w-[600px] gap-8 flex flex-col">
					{!error && thread && uri && <Snippet thread={thread} colorMode={colorMode} />}
...
function Snippet({
	...
	const snippet = useMemo(() => {
		...
		const profileHref = toShareUrl(["/profile", thread.post.author.did].join("/"));
		const urip = new AtUri(thread.post.uri);
		const href = toShareUrl(["/profile", thread.post.author.did, "post", urip.rkey].join("/"));
		...
		return `<blockquote class="bluesky-embed" data-bluesky-uri="${escapeHtml(
			thread.post.uri,
		)}" data-bluesky-cid="${escapeHtml(thread.post.cid)}" data-bluesky-embed-color-mode="${escapeHtml(
			colorMode,
