import { type AppBskyFeedDefs, AppBskyFeedPost, AppBskyRichtextFacet, RichText } from "@atproto/api";
		...
		<Container href={href}>
			<div className="flex-1 flex-col flex gap-2" lang={record?.langs?.[0]}>
				<div className="flex gap-2.5 items-center cursor-pointer">
					...
					<Link href={href} className="transition-transform hover:scale-110 shrink-0 self-start">
				...
				<div className="border-t dark:border-slate-600 w-full pt-2.5 flex items-center gap-5 text-sm cursor-pointer">
					...
					<div className="flex items-center gap-2 cursor-pointer">
						...
						<p className="font-bold text-neutral-500 dark:text-neutral-300 mb-px">Reply</p>
					</div>
					<div className="flex-1" />
...
function PostContent({ record }: { record: AppBskyFeedPost.Record | null }) {
	...
	return <p className="min-[300px]:text-lg leading-6 break-word break-words whitespace-pre-wrap">{richText}</p>;
}
