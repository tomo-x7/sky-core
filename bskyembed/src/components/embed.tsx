...
export function Embed({
	...
	try {
				...
				return (
					...
					>
						...
						{record.embeds?.map((embed) => (
							<Embed key={embed.$type} content={embed} labels={record.labels} hideRecord />
						))}
					</Link>
				);
			}
		...
		if (AppBskyEmbedRecordWithMedia.isView(content) && AppBskyEmbedRecord.isViewRecord(content.record.record)) {
			return (
				<div className="flex flex-col gap-2">
					<Embed content={content.media} labels={labels} hideRecord={hideRecord} />
					<Embed
						content={{
		...
		return <Info>{err instanceof Error ? err.message : "An error occurred"}</Info>;
	}
}
...
function ExternalEmbed({
	...
	return (
		<Link
			...
			disableTracking
		>
			{content.external.thumb && <img src={content.external.thumb} className="aspect-[1.91/1] object-cover" />}
...
function GenericWithImageEmbed({
	...
	return (
		<Link href={href} className="w-full rounded-lg border dark:border-slate-600 py-2 px-3 flex flex-col gap-2">
			<div className="flex gap-2.5 items-center">
				...
				<div className="flex-1">
					...
					<p className="text-textLight dark:text-textDimmed text-sm">{subtitle}</p>
				</div>
			</div>
			{description && <p className="text-textLight dark:text-textDimmed text-sm">{description}</p>}
		</Link>
	);
}
...
function getStarterPackHref(starterPack: AppBskyGraphDefs.StarterPackViewBasic) {
	const rkey = getRkey({ uri: starterPack.uri });
