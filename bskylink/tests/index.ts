...
import { Database, LinkService, envToCfg, readEnv } from "../src/index.js";
...
describe("link service", async () => {
	...
	it("serves permanent redirect, preserving query params.", async () => {
		...
		assert.strictEqual(locationUrl.pathname + locationUrl.search, "/start/did:example:carol/zzz?a=b");
	});
	...
	it("returns 404 for unknown link when requesting json", async () => {
		const [status, json] = await getJsonRedirect("https://test.bsky.link/unknown");
		assert(json.error);
		assert(json.message);
	...
	async function getRedirect(link: string): Promise<[number, string]> {
		...
		assert(res.status === 301 || res.status === 303, "response was not a redirect");
	...
	async function getJsonRedirect(
		...
		assert(res.headers.get("content-type")?.startsWith("application/json"), "content type was not json");
