...
import type { ScrollEventHandlerCallbackType, ScrollEventsHandlersHookType } from "../types";
...
export const useScrollEventsHandlersDefault: ScrollEventsHandlersHookType = (
	...
	//#region callbacks
	const handleOnScroll: ScrollEventHandlerCallbackType<ScrollEventContextType> = useWorkletCallback(
		(event, context) => {
			/**
	...
	const handleOnBeginDrag: ScrollEventHandlerCallbackType<ScrollEventContextType> = useWorkletCallback(
		(event, context) => {
	...
	const handleOnEndDrag: ScrollEventHandlerCallbackType<ScrollEventContextType> = useWorkletCallback(
	...
	const handleOnMomentumEnd: ScrollEventHandlerCallbackType<ScrollEventContextType> = useWorkletCallback(
		({ contentOffset: { y } }, context) => {
			if (animatedScrollableState.value === SCROLLABLE_STATE.LOCKED) {
				if (!(preserveScrollMomentum && isScrollEnded.value)) {
					const lockPosition = context.shouldLockInitialPosition ? (context.initialContentOffsetY ?? 0) : 0;
