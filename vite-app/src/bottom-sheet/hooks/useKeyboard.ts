...
import { Keyboard, type KeyboardEvent, type KeyboardEventEasing, type KeyboardEventName, Platform } from "react-native";
...
export const useKeyboard = () => {
	...
	const keyboardState = useSharedValue<KEYBOARD_STATE>(KEYBOARD_STATE.UNDETERMINED);
	...
	const handleKeyboardEvent = useWorkletCallback(
		(state: KEYBOARD_STATE, height: number, duration: number, easing: KeyboardEventEasing) => {
			...
			keyboardHeight.value =
				state === KEYBOARD_STATE.SHOWN ? height : height === 0 ? keyboardHeight.value : height;
