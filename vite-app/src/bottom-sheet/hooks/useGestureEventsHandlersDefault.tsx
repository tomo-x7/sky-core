...
import type { GestureEventHandlerCallbackType, GestureEventsHandlersHookType } from "../types";
...
export const useGestureEventsHandlersDefault: GestureEventsHandlersHookType = () => {
	...
	const handleOnActive: GestureEventHandlerCallbackType<GestureEventContextType> = useWorkletCallback(
		function handleOnActive(source, { translationY }, context) {
			...
			const negativeScrollableContentOffset =
				(context.initialPosition === highestSnapPoint && source === GESTURE_SOURCE.SCROLLABLE) ||
			...
			if (enableOverDrag) {
				if (
					(source === GESTURE_SOURCE.HANDLE || animatedScrollableType.value === SCROLLABLE_TYPE.VIEW) &&
					draggedPosition < highestSnapPoint
				) {
					const resistedPosition =
						highestSnapPoint -
						Math.sqrt(1 + (highestSnapPoint - draggedPosition)) * overDragResistanceFactor;
				...
				if (source === GESTURE_SOURCE.HANDLE && draggedPosition > lowestSnapPoint) {
					const resistedPosition =
						lowestSnapPoint + Math.sqrt(1 + (draggedPosition - lowestSnapPoint)) * overDragResistanceFactor;
					animatedPosition.value = resistedPosition;
					return;
				}
				...
				if (
					...
					const resistedPosition =
						lowestSnapPoint +
						Math.sqrt(1 + (draggedPosition + negativeScrollableContentOffset - lowestSnapPoint)) *
							overDragResistanceFactor;
					animatedPosition.value = resistedPosition;
					return;
				}
	...
	const handleOnEnd: GestureEventHandlerCallbackType<GestureEventContextType> = useWorkletCallback(
		function handleOnEnd(source, { translationY, absoluteY, velocityY }, context) {
			...
			if (
				 ...
				 */
				if (
					!(Platform.OS === "ios" && isScrollable && absoluteY > WINDOW_HEIGHT - animatedKeyboardHeight.value)
				) {
			...
			const wasGestureHandledByScrollView =
				source === GESTURE_SOURCE.SCROLLABLE && animatedScrollableContentOffsetY.value > 0;
			/**
