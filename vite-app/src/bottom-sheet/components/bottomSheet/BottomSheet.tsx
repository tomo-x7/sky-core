...
const BottomSheetComponent = forwardRef<BottomSheet, BottomSheetProps>(function BottomSheet(props, ref) {
	...
	const animatedHandleHeight = useReactiveSharedValue(_providedHandleHeight ?? INITIAL_HANDLE_HEIGHT);
	...
	const animatedSheetState = useDerivedValue(() => {
		...
		const extendedPositionWithKeyboard = Math.max(
			0,
			animatedContainerHeight.value - (animatedSheetHeight.value + keyboardHeightInContainer),
		);
	...
	const animatedIndex = useDerivedValue(() => {
		...
		const currentIndex = isLayoutCalculated.value
			? interpolate(animatedPosition.value, adjustedSnapPoints, adjustedSnapPointsIndexes, Extrapolate.CLAMP)
			: -1;
	...
	const handleOnAnimate = useCallback(
		function handleOnAnimate(toPoint: number, source: ANIMATION_SOURCE, snapPoints: number[]) {
			...
			const toIndex = toPoint === closedPosition ? -1 : snapPoints.indexOf(toPoint);
		...
		[_providedOnAnimate, animatedSnapPoints, animatedClosedPosition, animatedCurrentIndex],
	);
	//#endregion
	...
	useAnimatedReaction(
		...
		(result, _previousResult) => {
			...
			animatedKeyboardHeightInContainer.value = $modal
				? Math.abs(_keyboardHeight - Math.abs(bottomInset - animatedContainerOffset.value.bottom))
			...
			if (
				 ...
				 */
				(_keyboardState === KEYBOARD_STATE.HIDDEN && keyboardBlurBehavior === KEYBOARD_BLUR_BEHAVIOR.none) ||
				/**
					...
					>
						<Animated.View style={containerStyle}>
							<BottomSheetBackgroundContainer
								...
								backgroundStyle={_providedBackgroundStyle}
							/>
							<Animated.View pointerEvents="box-none" style={contentMaskContainerStyle}>
								...
								>
									...
									{renderFooter && <BottomSheetFooterContainer renderFooter={renderFooter} />}
								</BottomSheetDraggableView>
							</Animated.View>
