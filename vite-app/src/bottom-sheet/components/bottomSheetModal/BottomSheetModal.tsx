...
const BottomSheetModalComponent = forwardRef<BottomSheetModal, BottomSheetModalProps>(
	function BottomSheetModal(props, ref) {
		...
		//#region hooks
		const { containerHeight, containerOffset, mountSheet, unmountSheet, willUnmountSheet } =
		...
		const handleSnapToIndex = useCallback<BottomSheetMethods["snapToIndex"]>((...args) => {
			if (minimized.current) {
				return;
			}
			bottomSheetRef.current?.snapToIndex(...args);
		}, []);
		const handleSnapToPosition = useCallback<BottomSheetMethods["snapToPosition"]>((...args) => {
			if (minimized.current) {
				return;
			}
		...
		const handleCollapse = useCallback<BottomSheetMethods["collapse"]>((...args: any[]) => {
			if (minimized.current) {
				return;
			}
		...
		const handleForceClose = useCallback<BottomSheetMethods["forceClose"]>((...args: any[]) => {
			if (minimized.current) {
				return;
			}
		...
		const handlePortalRender = useCallback(function handlePortalRender(render: () => void) {
			if (mounted.current) {
				render();
			}
		}, []);
			...
			>
				<ContainerComponent key={key}>
					<BottomSheet
						...
						children={typeof Content === "function" ? <Content data={data} /> : Content}
						$modal={true}
					/>
				</ContainerComponent>
			</Portal>
