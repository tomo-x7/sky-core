...
export function show(message: string, icon: FontAwesomeProps["icon"] = "check") {
	if (process.env.NODE_ENV === "test") {
		return;
	}
	AccessibilityInfo.announceForAccessibility(message);
	const item = new RootSiblings(<Toast message={message} icon={icon} destroy={() => item.destroy()} />);
}
...
function Toast({
	...
	}, [dismissSwipeTranslateY, isPanning, alive, hideAndDestroyAfterTimeout, pauseDestroy]);

	const topOffset = top + 10;

	useAnimatedReaction(
		() => !isPanning.get() && dismissSwipeTranslateY.get() < -topOffset - cardHeight,
	...
	return (
		<GestureHandlerRootView style={[a.absolute, { top: topOffset, left: 16, right: 16 }]} pointerEvents="box-none">
			{alive && (
				<Animated.View entering={FadeInUp} exiting={FadeOutUp} style={[a.flex_1]}>
					<Animated.View
							...
							<View style={[a.flex_1, a.px_md, a.py_lg, a.flex_row, a.gap_md]}>
								<View
									style={[
										...
										a.justify_center,
										{
											backgroundColor: t.name === "dark" ? t.palette.black : t.palette.primary_50,
										},
									]}
								>
									<FontAwesomeIcon icon={icon} size={16} style={t.atoms.text_contrast_medium} />
								</View>
