...
export function TabBar({
	...
	const syncScrollState = useSharedValue<"synced" | "unsynced" | "needs-sync">("synced");
	...
	useAnimatedReaction(
		...
		(nextLayoutsLength, prevLayoutsLength) => {
			if (nextLayoutsLength !== prevLayoutsLength) {
				if (nextLayoutsLength === itemsLength && didInitialScroll.get() === false) {
	...
	useAnimatedReaction(
		...
		(nextDragState, prevDragState) => {
			if (
				nextDragState !== prevDragState &&
				nextDragState === "idle" &&
				(syncScrollState.get() === "unsynced" || syncScrollState.get() === "needs-sync")
			) {
				const progress = dragProgress.get();
				const offset = progressToOffset(progress);
	...
	const onPressUIThread = useCallback(
		(index: number) => {
			...
			if (syncScrollState.get() === "synced" || syncScrollState.get() === "needs-sync" || scrollIntoView) {
		...
		[syncScrollState, scrollElRef, scrollX, progressToOffset, containerSize, layouts],
	);
	...
	const indicatorStyle = useAnimatedStyle(() => {
		...
		if (layoutsValue.length !== itemsLength || textLayoutsValue.length !== itemsLength) {
			return {
				opacity: 0,
			};
		}
	...
	return (
		<View testID={testID} style={[t.atoms.bg, a.flex_row]} accessibilityRole="tablist">
			<BlockDrawerGesture>
				<ScrollView
