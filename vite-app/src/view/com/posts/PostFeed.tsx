...
let PostFeed = ({
	...
	const { data, isFetching, isFetched, isError, error, refetch, hasNextPage, isFetchingNextPage, fetchNextPage } =
	...
	const onPostCreated = React.useCallback(() => {
		...
		// -prf
		if (data?.pages.length === 1 && (feed === "following" || feed === `author|${myDid}|posts_and_author_threads`)) {
	...
	React.useEffect(() => {
		if (enabled && !disablePoll) {
			...
			if ((isEmpty || timeSinceFirstLoad > CHECK_LATEST_AFTER) && checkForNewRef.current) {
	...
	const showProgressIntersitial = (followProgressGuide || followAndLikeProgressGuide) && !rightNavVisible;
	...
	const renderItem = React.useCallback(
		({ item: row, index: rowIndex }: ListRenderItemInfo<FeedRow>) => {
			if (row.type === "empty") {
				...
				return <PostFeedVideoGridRow items={row.items} sourceContext={sourceContext} />;
			} else {
				return null;
			}
		},
