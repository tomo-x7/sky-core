...
export class Convo {
	...
	private pastMessages: Map<string, ChatBskyConvoDefs.MessageView | ChatBskyConvoDefs.DeletedMessageView> = new Map();
	private newMessages: Map<string, ChatBskyConvoDefs.MessageView | ChatBskyConvoDefs.DeletedMessageView> = new Map();
	private pendingMessages: Map<string, { id: string; message: ChatBskyConvoSendMessage.InputSchema["message"] }> =
		new Map();
	...
	async fetchMessageHistory() {
		...
		try {
			...
			for (const message of messages) {
				if (ChatBskyConvoDefs.isMessageView(message) || ChatBskyConvoDefs.isDeletedMessageView(message)) {
					/*
		...
		for (const ev of events) {
			...
			if ("rev" in ev && typeof ev.rev === "string") {
				...
				if (isNewEvent || isUninitialized) {
					...
					if (ChatBskyConvoDefs.isLogCreateMessage(ev) && ChatBskyConvoDefs.isMessageView(ev.message)) {
						/**
				...
				switch (e.message) {
					case "block between recipient and sender":
						this.emitter.emit("event", {
							...
							accountDids: [this.sender!.did, ...this.recipients!.map((r) => r.did)],
						});
						break;
