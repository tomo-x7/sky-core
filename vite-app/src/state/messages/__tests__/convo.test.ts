...
describe(`#/state/messages/convo`, () => {
	...
	describe(`sending messages`, () => {
		...
		it.todo(`successfully sent messages are re-ordered, if needed, by events received from server`);
	...
	describe(`inactivity`, () => {
		it.todo(`below a certain threshold of inactivity, restore entirely from log`);
		it.todo(`above a certain threshold of inactivity, rehydrate entirely fresh state`);
	});
});
