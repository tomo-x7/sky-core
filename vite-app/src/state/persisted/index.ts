...
export async function write<K extends keyof Schema>(key: K, value: Schema[K]): Promise<void> {
...
async function onBroadcastMessage({ data }: MessageEvent) {
	if (
		...
		if (next) {
			...
			console.error("persisted state: handled update update from broadcast channel, but found no data");
		}
	}
}
