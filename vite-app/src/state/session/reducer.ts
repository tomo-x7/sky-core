...
let reducer = (state: State, action: Action): State => {
	switch (action.type) {
		case "received-agent-event": {
			...
			if (refreshedAccount === undefined && agent !== state.currentAgentState.agent) {
			...
			if (!existingAccount || JSON.stringify(existingAccount) === JSON.stringify(refreshedAccount)) {
			...
			return {
				...
				currentAgentState: refreshedAccount ? state.currentAgentState : createPublicAgentState(), // Log out if expired.
				needsPersist: true,
			};
		}
		case "switched-to-account": {
			...
			return {
				accounts: [newAccount, ...state.accounts.filter((a) => a.did !== newAccount.did)],
				currentAgentState: {
					did: newAccount.did,
					agent: newAgent,
				},
