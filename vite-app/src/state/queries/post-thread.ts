	...
	return useQuery<PostThreadQueryData, Error>({
		...
		async queryFn() {
			...
			if (res.success) {
				...
				return {
					thread,
					threadgate: res.data.threadgate as AppBskyFeedDefs.ThreadgateView | undefined,
				};
			}
	...
	if (node.replies) {
		node.replies.sort((a: ThreadNode, b: ThreadNode) => {
			...
			if (node.ctx.isHighlightedPost || opts.lab_treeViewEnabled) {
				const aIsJustPosted = a.post.author.did === currentDid && justPostedUris.has(a.post.uri);
				const bIsJustPosted = b.post.author.did === currentDid && justPostedUris.has(b.post.uri);
...
function getHotness(threadPost: ThreadPost, fetchedAt: number) {
	...
	const hoursAgo = Math.max(
		0,
		(new Date(fetchedAt).getTime() - new Date(post.indexedAt).getTime()) / (1000 * 60 * 60),
	);
...
function responseToThreadNodes(
	...
	if (
		...
		bsky.dangerousIsType<AppBskyFeedPost.Record>(node.post.record, AppBskyFeedPost.isRecord)
	) {
		...
		return {
			...
			parent:
				node.parent && direction !== "down" ? responseToThreadNodes(node.parent, depth - 1, "up") : undefined,
			replies:
			...
			ctx: {
				...
				hasMore: direction === "down" && !node.replies?.length && !!post.replyCount,
...
function annotateSelfThread(thread: ThreadNode) {
	...
	let node = thread;
	for (let i = 0; i < 10; i++) {
		const reply = node.replies?.find((r) => r.type === "post" && r.post.author.did === thread.post.author.did);
		if (reply?.type !== "post") {
			break;
		}
...
export function* findAllPostsInQueryData(queryClient: QueryClient, uri: string): Generator<ThreadNode, void> {
	...
	for (const [_queryKey, queryData] of queryDatas) {
		...
		for (const item of traverseThread(thread)) {
			...
			const quotedPost = item.type === "post" ? getEmbeddedPost(item.post.embed) : undefined;
...
function threadNodeToPlaceholderThread(node: ThreadNode): ThreadNode | undefined {
	if (node.type !== "post") {
		return undefined;
	}
	return {
...
function postViewToPlaceholderThread(post: AppBskyFeedDefs.PostView): ThreadNode {
	return {
		type: "post",
		_reactKey: post.uri,
		uri: post.uri,
...
function embedViewRecordToPlaceholderThread(record: AppBskyEmbedRecord.ViewRecord): ThreadNode {
	return {
		type: "post",
