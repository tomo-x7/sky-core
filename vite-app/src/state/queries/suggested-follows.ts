...
const suggestedFollowsQueryKey = (options?: SuggestedFollowsOptions) => [suggestedFollowsQueryKeyRoot, options];

const suggestedFollowsByActorQueryKeyRoot = "suggested-follows-by-actor";
const suggestedFollowsByActorQueryKey = (did: string) => [suggestedFollowsByActorQueryKeyRoot, did];
...
export function useSuggestedFollowsQuery(options?: SuggestedFollowsOptions) {
	...
	>({
		...
		queryFn: async ({ pageParam }) => {
			...
			const maybeDifferentLimit = options?.subsequentPageLimit && pageParam ? options.subsequentPageLimit : limit;
			...
			res.data.actors = res.data.actors
				.filter((actor) => !moderateProfile(actor, moderationOpts!).ui("profileList").filter)
				.filter((actor) => {
