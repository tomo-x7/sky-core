...
export type InviteCodesQueryResponse = Exclude<ReturnType<typeof useInviteCodesQuery>["data"], undefined>;
export function useInviteCodesQuery() {
	...
	return useQuery({
		...
		queryFn: async () => {
			const res = await agent.com.atproto.server.getAccountInviteCodes({}).catch((e) => {
			...
			const used = res.data.codes
				...
				.sort((a, b) => {
					return new Date(b.uses[0].usedAt).getTime() - new Date(a.uses[0].usedAt).getTime();
				});
