...
export function VideoFeed({}: NativeStackScreenProps<CommonNavigatorParams, "VideoFeed">) {
	...
	return (
		<ThemeProvider theme="dark">
			<Layout.Screen noInsetTop style={{ backgroundColor: "black" }}>
				<View style={[a.absolute, a.z_50, { top: 0, left: 0, right: 0, paddingTop: top }]}>
...
function Feed() {
	...
	const feedDesc = useMemo(() => {
		switch (params.type) {
			...
			case "author":
				return `author|${params.did as string}|${params.filter as AuthorFilter}` as const;
			default:
				throw new Error(`Invalid video feed params ${JSON.stringify(params)}`);
		}
	}, [params]);
	const feedFeedback = useFeedFeedback(feedDesc, hasSession);
	const { data, error, hasNextPage, isFetchingNextPage, fetchNextPage } = usePostFeedQuery(
		feedDesc,
	...
	const [currentSources, setCurrentSources] = useState<[CurrentSource, CurrentSource, CurrentSource]>([
		null,
		null,
		null,
	]);

	const [players, setPlayers] = useState<[VideoPlayer, VideoPlayer, VideoPlayer] | null>(null);
		...
		({ item, index }) => {
			...
			return (
				<VideoItem
					...
					post={post}
					embed={video}
					active={isFocused && index === currentIndex && currentSource?.source === video.playlist}
	...
	const updateVideoState = useCallback(
		(index: number) => {
			...
			const prevVideo = prevEmbed && AppBskyEmbedVideo.isView(prevEmbed) ? prevEmbed.playlist : null;
			const currSlice = videos.at(index);
			const currPost = currSlice?.post;
			const currEmbed = currPost?.embed;
			const currVideo = currEmbed && AppBskyEmbedVideo.isView(currEmbed) ? currEmbed.playlist : null;
			...
			const nextVideo = nextEmbed && AppBskyEmbedVideo.isView(nextEmbed) ? nextEmbed.playlist : null;
			...
			if (!players) {
				...
				if (currVideo) {
					...
					if (
						currVideoModeration &&
						(currVideoModeration.ui("contentView").blur || currVideoModeration.ui("contentMedia").blur)
					) {
						currPlayer.pause();
					} else {
						currPlayer.play();
			...
			const updatedSources: [CurrentSource, CurrentSource, CurrentSource] = [...currentSources];
			if (prevVideo && prevVideo !== prevPlayerCurrentSource?.source) {
		...
		<View style={[a.relative, { height, width }]}>
			{postShadow === POST_TOMBSTONE ? (
				...
				<>
					...
					{shouldRenderVideo && player && <VideoItemInner player={player} embed={embed} />}
					{moderation && (
						<Overlay
							player={player}
							post={postShadow}
	...
	return (
		<View style={[a.absolute, a.inset_0, a.z_20]}>
			...
			>
				...
				>
					<LinearGradient colors={["rgba(0,0,0,0)", "rgba(0,0,0,0.4)"]} style={[a.absolute, a.inset_0]} />
...
function Overlay({
	...
	const record = bsky.dangerousIsType<AppBskyFeedPost.Record>(post.record, AppBskyFeedPost.isRecord)
		? post.record
		: undefined;
	...
	return (
		<Hider.Outer modui={mergedModui}>
			...
			<Hider.Content>
				<View style={[a.absolute, a.inset_0, a.z_20]}>
					<View style={[a.flex_1]}>
						{player && <PlayPauseTapArea player={player} post={post} feedContext={feedContext} />}
					</View>

					<LinearGradient
						colors={["rgba(0,0,0,0)", "rgba(0,0,0,0.7)", "rgba(0,0,0,0.95)", "rgba(0,0,0,0.95)"]}
					...
					>
						<Animated.View style={[a.px_md, animatedStyle]}>
							<View style={[a.w_full, a.flex_row, a.align_center, a.gap_md]}>
								...
								>
									...
									<View style={[a.flex_1]}>
										...
										<Text style={[a.text_sm, t.atoms.text_contrast_high]} numberOfLines={1}>
											{handle}
										</Text>
									</View>
								</Link>
								...
								{post.author.did !== currentAccount?.did && !post.author.viewer?.following && (
									<Button
										...
										color="secondary_inverted"
										style={[a.mb_xs]}
										onPress={() => (profile.viewer?.following ? queueUnfollow() : queueFollow())}
									>
							...
							{record?.text?.trim() && (
								<ExpandableRichTextView value={richText} authorHandle={post.author.handle} />
							)}
							{record && (
								<View style={[{ left: -5 }]}>
									<PostCtrls
...
function ExpandableRichTextView({
	...
	return (
		<ScrollView
			...
			contentContainerStyle={[a.py_sm, a.gap_xs, expanded ? [a.align_start] : a.flex_row]}
		>
			<RichText
				value={value}
				...
				enableTags
				numberOfLines={expanded || screenReaderEnabled ? undefined : constrained ? 2 : 2}
				onTextLayout={(evt) => {
...
function VideoItemPlaceholder({
	...
	let contentFit = isTallAspectRatio(embed.aspectRatio) ? ("cover" as const) : ("contain" as const);
	if (blur) {
