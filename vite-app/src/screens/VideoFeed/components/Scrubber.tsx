...
export function Scrubber({
	...
	const barStyle = useAnimatedStyle(() => {
		const currentTime = isSeekingSV.get() ? seekProgressSV.get() : currentTimeSV.get();
	...
	return (
		<>
			{player && active && <PlayerListener player={player} setDuration={setDuration} updateTime={updateTime} />}
			<Animated.View
				style={[
					a.absolute,
					{
			...
			>
				<Text style={[a.text_center, a.font_bold]}>
					<Text style={[a.text_5xl, { fontVariant: ["tabular-nums"] }]}>{formatTime(currentSeekTime)}</Text>
					<Text style={[a.text_2xl, { opacity: 0.8 }]}>{"  /  "}</Text>
					<Text style={[a.text_5xl, { opacity: 0.8 }, { fontVariant: ["tabular-nums"] }]}>
						{formatTime(duration)}
					</Text>
				</Text>
			</Animated.View>
			...
			<GestureDetector gesture={scrubPanGesture}>
				...
				>
					<View style={[a.w_full, a.relative]}>
						<Animated.View style={[a.w_full, { backgroundColor: "white", opacity: 0.2 }, trackStyle]} />
						<Animated.View style={[a.absolute, { top: 0, left: 0, backgroundColor: "white" }, barStyle]} />
					</View>
					<Animated.View style={[{ minHeight: VIDEO_PLAYER_BOTTOM_INSET }, childrenStyle]}>
						{children}
					</Animated.View>
				</View>
			</GestureDetector>
