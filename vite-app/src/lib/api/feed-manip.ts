...
export class FeedViewPostsSlice {
	...
	constructor(feedPost: FeedViewPost) {
		...
		if (!AppBskyFeedPost.isRecord(post.record) || !bsky.validate(post.record, AppBskyFeedPost.validateRecord)) {
			return;
		}
	...
	get reason() {
		return "__source" in this._feedPost ? (this._feedPost.__source as ReasonFeedSource) : this._feedPost.reason;
	}
...
export class FeedTuner {
	...
	static removeReplies(tuner: FeedTuner, slices: FeedViewPostsSlice[], _dryRun: boolean) {
	...
	static removeReposts(tuner: FeedTuner, slices: FeedViewPostsSlice[], _dryRun: boolean) {
		for (let i = 0; i < slices.length; i++) {
	...
	static removeQuotePosts(tuner: FeedTuner, slices: FeedViewPostsSlice[], _dryRun: boolean) {
		for (let i = 0; i < slices.length; i++) {
	...
	static removeOrphans(tuner: FeedTuner, slices: FeedViewPostsSlice[], _dryRun: boolean) {
		for (let i = 0; i < slices.length; i++) {
	...
	static followedRepliesOnly({ userDid }: { userDid: string }) {
		return (tuner: FeedTuner, slices: FeedViewPostsSlice[], _dryRun: boolean): FeedViewPostsSlice[] => {
			for (let i = 0; i < slices.length; i++) {
				const slice = slices[i];
...
function shouldDisplayReplyInFollowing(authors: AuthorContext, userDid: string): boolean {
	...
	if (grandparentAuthor && grandparentAuthor.did !== author.did && isSelfOrFollowing(grandparentAuthor, userDid)) {
		return true;
	}
