...
export async function compressIfNeeded(img: Image, maxSize = 1000000): Promise<Image> {
	...
	const finalImageMovedPath = await moveToPermanentPath(resizedImage.path, ".jpg");
	const finalImg = {
		...resizedImage,
...
async function doResize(localUri: string, opts: DoResizeOpts): Promise<Image> {
	...
	for (let i = 0; i < 9; i++) {
		...
		const resizeRes = await manipulateAsync(localUri, [{ resize: newDimensions }], {
	...
	throw new Error(`This image is too big! We couldn't compress it down to ${opts.maxSize} bytes`);
}
...
export async function saveBytesToDisk(filename: string, bytes: Uint8Array, type: string) {
	const encoded = Buffer.from(bytes).toString("base64");
	return await saveToDevice(filename, encoded, type);
}

export async function saveToDevice(filename: string, encoded: string, type: string) {
	try {
		if (isIOS) {
			...
			const fileUrl = await StorageAccessFramework.createFileAsync(permissions.directoryUri, filename, type);
...
async function withTempFile<T>(filename: string, encoded: string, cb: (url: string) => T | Promise<T>): Promise<T> {
...
export function getResizedDimensions(originalDims: {
	...
	if (originalDims.width <= POST_IMG_MAX.width && originalDims.height <= POST_IMG_MAX.height) {
		return originalDims;
	}

	const ratio = Math.min(POST_IMG_MAX.width / originalDims.width, POST_IMG_MAX.height / originalDims.height);
