	...
	return useMemo(
		() => ({
			...
			"!warn": {
				...
				description: _(msg`This content has received a general warning from moderators.`),
			},
			"!no-unauthenticated": {
				name: _(msg`Sign-in Required`),
				description: _(msg`This user has requested that their content only be shown to signed-in users.`),
			},
			porn: {
