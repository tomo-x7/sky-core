...
export const BUNDLE_DATE = IS_TESTFLIGHT || __DEV__ ? 0 : Number(process.env.EXPO_PUBLIC_BUNDLE_DATE);

export const appVersion = `${nativeApplicationVersion}.${nativeBuildVersion}`;
export const bundleInfo = `${BUNDLE_IDENTIFIER} (${__DEV__ ? "dev" : IS_TESTFLIGHT ? "tf" : "prod"})`;
