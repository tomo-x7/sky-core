...
export async function getLinkMeta(agent: BskyAgent, url: string, timeout = 15e3): Promise<LinkMeta> {
	...
	try {
		...
		const response = await fetch(`${LINK_META_PROXY(agent.service.toString() || "")}${encodeURIComponent(url)}`, {
