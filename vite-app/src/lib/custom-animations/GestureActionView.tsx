...
export function GestureActionView({
	...
	if ((actions.leftSecond && !actions.leftFirst) || (actions.rightSecond && !actions.rightFirst)) {
	...
	const runPopAnimation = () => {
		...
		iconScale.set(() => withSequence(withTiming(1.2, { duration: 175 }), withTiming(1, { duration: 100 })));
	};
	...
	useAnimatedReaction(
		...
		() => {
			if (transX.get() === 0) {
				...
				if (actions.leftSecond && transX.get() <= -actions.leftSecond.threshold) {
				...
				if (actions.rightSecond && transX.get() > actions.rightSecond.threshold) {
	...
	const interpolation = React.useMemo<{
		...
		if (!actions.leftFirst) {
			...
			return {
				inputRange: [...leftSideInterpolation.inputRange, ...rightSideInterpolation.inputRange],
				outputRange: [...leftSideInterpolation.outputRange, ...rightSideInterpolation.outputRange],
			};
		}
	}, [leftSideInterpolation, rightSideInterpolation, actions.leftFirst, actions.rightFirst]);
						...
						<Animated.View style={animatedIconStyle}>
							{activeAction === "leftFirst" && actions.leftFirst?.icon ? (
								...
								<actions.rightFirst.icon
									...
									style={{ color: "white" }}
								/>
							) : activeAction === "rightSecond" && actions.rightSecond?.icon ? (
