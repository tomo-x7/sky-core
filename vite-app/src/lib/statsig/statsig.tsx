...
function createStatsigOptions(prefetchUsers: StatsigUser[]) {
	return {
		environment: {
			tier: process.env.NODE_ENV === "development" ? "development" : IS_TESTFLIGHT ? "staging" : "production",
		},
...
export function attachRouteToLogEvents(getRouteName: () => string | null | undefined) {
 ...
 */
export function useDangerousSetGate(): (gateName: Gate, value: boolean) => void {
	const cache = React.useContext(GateCache);
	if (!cache) {
...
export async function tryFetchGates(did: string | undefined, strategy: "prefer-low-latency" | "prefer-fresh-gates") {
	try {
		...
		if (Statsig.initializeCalled()) {
			await Promise.race([timeout(timeoutMs), Statsig.prefetchUsers([toStatsigUser(did)])]);
		}
	} catch (e) {
