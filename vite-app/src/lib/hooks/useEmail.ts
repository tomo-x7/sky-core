...
export function useEmail() {
	...
	const isNewEnough = !!profile?.createdAt && Date.parse(profile.createdAt) >= Date.parse("2024-11-18T19:00:00.000Z");

	const isSelfHost =
		currentAccount && getHostnameFromUrl(currentAccount.service) !== getHostnameFromUrl(BSKY_SERVICE);

	const needsEmailVerification = !isSelfHost && checkEmailConfirmed && !currentAccount?.emailConfirmed && isNewEnough;
