...
cfg.resolver.resolveRequest = (context, moduleName, platform) => {
	...
	if (moduleName.startsWith("multiformats/hashes/hasher")) {
		return context.resolveRequest(context, "multiformats/dist/src/hashes/hasher", platform);
	}
	if (moduleName.startsWith("multiformats/cid")) {
		return context.resolveRequest(context, "multiformats/dist/src/cid", platform);
	}
	...
	if (process.env.BSKY_PROFILE) {
		if (moduleName.endsWith("ReactNativeRenderer-prod")) {
			return context.resolveRequest(context, moduleName.replace("-prod", "-profiling"), platform);
		}
	}
