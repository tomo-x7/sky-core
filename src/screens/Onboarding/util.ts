...
export async function bulkWriteFollows(agent: BskyAgent, dids: string[]) {
	...
	const followWrites: $Typed<ComAtprotoRepoApplyWrites.Create>[] = followRecords.map((r) => ({
	...
	for (const r of followWrites) {
		followUris.set(r.value.subject, `at://${session.did}/app.bsky.graph.follow/${r.rkey}`);
	}
	return followUris;
}
