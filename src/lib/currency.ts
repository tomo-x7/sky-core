 ...
 * API for region.
 */
export function useFormatCurrency(options?: Parameters<typeof Intl.NumberFormat>[1]) {
	const { geolocation } = useGeolocation();
	const { appLanguage } = useLanguagePrefs();
	return React.useMemo(() => {
		const locale = deviceLocales.at(0);
		const languageTag = locale?.languageTag || appLanguage || "en-US";
		const countryCode = (locale?.regionCode || geolocation?.countryCode || "us").toLowerCase();
